                        ...
                        <div class="row align-items-center">
                            ...
                            <div class="col-1">
                                <button class="btn text-danger remove-item" 
                                    ...
                                    <i style="color:#ab6752;" class="fas fa-trash"></i>
                                </button>
                            </div>
                        </div>
                    </div>
...
{{-- <div class="mb-4">
    ...
    <select class="form-select" id="deliveryLocation" onchange="updateCartTotal()">
        <option value="{{ $deliveryCharge1->charge }}">Inside Dhaka (৳{{ $deliveryCharge1->charge }})</option>
        <option value="{{ $deliveryCharge2->charge }}">Outside Dhaka (৳{{ $deliveryCharge2->charge }})</option>
    </select>
</div> --}}
                    <!-- Price Details -->
                    <div class="mb-4">
                        ...
                        {{-- <div class="d-flex justify-content-between mb-2">
                            <span>Shipping</span>
                            <span class="cart-shipping">৳{{ $cartItems->count() > 0 ? number_format(50.00, 2) : '0.00' }}</span>
                        </div> --}}
                        {{-- <div class="d-flex justify-content-between mb-2">
                            <span>Tax (10%)</span>
                        ...
                        <div class="d-flex justify-content-between mb-2">
                            ...
                            <strong class="cart-total">৳{{ number_format(
                                $cartItems->sum(function($item) {
                                    return $item->price * $item->quantity;
                                })   // Default to inside Dhaka delivery charge
                            ) }}</strong>
                        </div>
                    </div>

                    @if($cartItems->count() > 0)
                        <a href="{{ route('shipping') }}" class="btn  w-100 mb-3 checkout-btn" style="color:#f5f5f5;background-color:#ab6752;">
                            Proceed to Checkout
                        </a>
                    @endif
                </div>
...
function updateCartTotal() {
    ...
    // Convert delivery charge to number
    // const deliveryCharge = parseFloat(document.getElementById('deliveryLocation').value);
